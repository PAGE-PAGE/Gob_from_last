...
final_text = ["Огромное вам спасибо,",
              ...
              ""]
text_number = 0
f1 = pg.font.Font('fractal.otf', 30)

pg.mixer.music.load('sound.wav')
pg.mixer.music.set_volume(0.3)
pg.mixer.music.play()

lauser_sound = pg.mixer.Sound('lauser_sound.wav')
...
while is_running:
        ...
        if event.type == pg.KEYDOWN:
            ...
            if mode == 'moon':
                if event.key == pg.K_SPACE:
                    lausers.add(Laser(starship.rect.midtop))
                    lauser_sound.play()
            if mode == 'final_scene':
                text_number += 2
    ...
    if mode == "moon":
        ...
        for hit in hits:
            ...
            if heart_count <= 0:
                is_running = False
        hits = pg.sprite.groupcollide(mice, lausers, True, True)
        screen.blit(space, (0, 0))
        screen.blit(starship.image, starship.rect)
        mice.draw(screen)
        lausers.draw(screen)
